...
function SaveForm ({ item, onSubmit = (item) => { } }) {
  ...
  return (
    <form className="max-w-sm mx-auto" onSubmit={e => { e.preventDefault(); onSubmit(submittedItem) }}>
      <div className="mb-5">
        <label htmlFor="id" className="block mb-2 text-sm font-medium text-gray-900">Menu ID</label>
        <input value={item?.id || ''} disabled type="text" id="id" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
      </div>
      <div className="mb-5">
        <label htmlFor="depth" className="block mb-2 text-sm font-medium text-gray-900">Depth</label>
        <input value={item?.depth || 0} disabled type="number" id="depth" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
      </div>
      <div className="mb-5">
        <label htmlFor="parent" className="block mb-2 text-sm font-medium text-gray-900">Parent Data</label>
        <input value={item?.parent?.name || ''} disabled type="text" id="parent" className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" />
      </div>
      <div className="mb-5">
